...
options = {'face_field': 'age,gender,emotion', "max_face_num": 10}
...
video_capture = cv2.VideoCapture('3号视频reaction.mp4') #打开摄像头 index默认自带摄像头0
...
while (True):  # video_capture.isOpened()):
    ...
    process = False
    if ready % 20 <= 5:  # 每24次获得一次人脸框 ，这里不占时间，占时间的是后面的计算
        process = True
    ...
    if (result['error_code']) == 0 and process == True:  # 均衡,每30帧处理5次 5/30
        ...
        for item, item1 in zip(result['result']['face_list'], result1['result']['face_list']):
            ...
            cv2.rectangle(image, (int(bbox_head[0]*rate_w), int(bbox_head[1]*rate_h)), (int(bbox_head[2]*rate_w), int(bbox_head[3]*rate_h)),(255, 0, 0), 0)
            ...
            arrow_size = 200
            pt2 = (int(eyes[0] * WIDTH + arrow_size*(mygaze[0]-0.7)), int(eyes[1] * HEIGHT + arrow_size*(mygaze[1] -0.6)))
